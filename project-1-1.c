...
void* producer(void* _args) {
	...
	if((in + 1) % args->current_buffer_size != out){
		*(args->trips) = *(args->trips) + 1;
	}
	while ((((in + 1) % args->current_buffer_size) == out) && doneProducing == false);  // Wait for space in buffer
...
void* consumer(void* _args) {
	...
	while (true) { 
		...
		while ((in == out) && (doneProducing == false));
 		if ((in == out) && doneProducing) {
